...
export function DiffResultsPageHtmlTemplate(organization1: string, organization2: string, diffResultsItems: Array<string>): string {
    ...
    baseHtml = StringUtils.replaceAll(baseHtml, '{diffResultsItems}', diffResultsItems.join('<hr />'));
    baseHtml = StringUtils.replaceAll(baseHtml, '{organization1}', organization1);
    baseHtml = StringUtils.replaceAll(baseHtml, '{organization2}', organization2);
