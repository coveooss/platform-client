 ...
 *
 * @export
 * @class PageCssResourceController
 * @extends {BaseController}
 */
export class PageCssResourceController extends BaseController {
  ...
  runDiffSequence(diffOptions?: IDiffOptions): Promise<DiffResultArray<PageResource>> {
    ...
    return new Promise((resolve, reject) => {
      const diffResultArray = DiffUtils.getDiffResult(this.page1.getCss(), this.page2.getCss(), diffOptions);
  ...
  private graduateNew(diffResult: DiffResultArray<PageResource>): Promise<void[]> {
    ...
    const asyncArray = diffResult.TO_CREATE.map((pageResource: PageResource) => {
      return (callback: any) => {
        PageResourceAPI.createCssResource(this.organization2, this.page2.getId(), pageResource.getConfiguration())
  ...
  private graduateUpdated(diffResult: DiffResultArray<PageResource>): Promise<void[]> {
    ...
    const asyncArray = diffResult.TO_UPDATE.map((pageResource: PageResource, idx: number) => {
      return (callback: any) => {
        ...
        PageResourceAPI.updateCssResource(this.organization2, this.page2.getId(), resourceName, pageResource.getConfiguration())
          .then((response: RequestResponse) => {
            callback(null, response);
  ...
  private graduateDeleted(diffResult: DiffResultArray<PageResource>): Promise<void[]> {
    ...
    const asyncArray = diffResult.TO_DELETE.map((pageResource: PageResource) => {
      return (callback: any) => {
        PageResourceAPI.deleteCssResource(this.organization2, this.page2.getId(), pageResource.getName())
          .then((response: RequestResponse) => {
            callback(null, response);
