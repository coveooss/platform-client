  ...
  describe('Get Diff Result', () => {
    ...
    describe('Field Diff', () => {
      ...
      it('Should return the new items', () => {
        ...
        expect(map(diff.TO_CREATE, getConfiguration)).to.eql([
          [1, 'a'],
          [3, 'c'],
        ]);
      });
      ...
      it('Should return the updated items', () => {
        ...
        expect(diff.containsItems()).to.be.true;
        expect(map(diff.TO_UPDATE, getConfiguration)).to.eql([
          [1, 'a'],
          [2, 'b'],
          [4, 'd'],
        ]);
      ...
      it('Should return the old items (to be updated)', () => {
        const diff: DiffResultArray<ClonableTest> = DiffUtils.getDiffResult(dict1, dictUpdated);
        expect(diff.containsItems()).to.be.true;
        expect(map(diff.TO_UPDATE_OLD, getConfiguration)).to.eql([
          [1, 'aaa'],
          [2, 'bb'],
          [22, 'd'],
        ]);
      ...
      it('Should return the deleted items', () => {
        ...
        expect(diff.containsItems()).to.be.true;
        expect(map(diff.TO_DELETE, getConfiguration)).to.eql([
          [5, 'e'],
          [6, 'f'],
        ]);
      });
      ...
      it('Should return all modifications', () => {
        ...
        expect(map(diff.TO_CREATE, getConfiguration)).to.eql([[3, 'c']]);
        expect(map(diff.TO_UPDATE, getConfiguration)).to.eql([[1, 'a']]);
        expect(map(diff.TO_UPDATE_OLD, getConfiguration)).to.eql([[11, 'a']]);
        expect(map(diff.TO_DELETE, getConfiguration)).to.eql([
          [6, 'fire'],
          [27, 'ground'],
        ]);
